    ...
    pub fn parse_unary(&mut self) -> Result<ASTNode, Vec<SyntaxError>> {
        match self.advance().kind {
            ...
            Kind::False => {
                ...
                Ok(ASTNode::Literal(LiteralExprNode {
                    position: self.previous.position,
                    value: Variable::from_bool(false),
                    actual_type: Type::Boolean,
                }))
            }
            Kind::String => {
                ...
                Ok(ASTNode::Literal(LiteralExprNode {
                    position: self.previous.position,
                    value: Variable::from_string(self.previous.clone().lexeme),
            ...
            Kind::Integer => {
                ...
                Ok(ASTNode::Literal(LiteralExprNode {
                    position: self.previous.position,
                    value: Variable::from_int(self.previous.clone().lexeme.parse::<i64>().unwrap()),
            ...
            Kind::LeftParen => match self.parse_expression() {
                Ok(node) => match self.current.kind {
                    ...
                    _ => Err(vec![SyntaxError {
                        ...
                        description: "Missing closing bracket".to_string(),
                    }]),
                },
                Err(synerr) => Err(synerr),
            },
            other => Err(vec![SyntaxError {
                position: self.current.position,
                raw_line: self.scanner.curr_line(),
